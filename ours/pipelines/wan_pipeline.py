...
class WanVideoToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def encode_video(
        ...
        if isinstance(generator, list):
            init_latents = [
                retrieve_latents(self.vae.encode(video[i].unsqueeze(0)), generator[i], sample_mode='argmax') for i in range(batch_size)
            ]
        else:
            init_latents = [retrieve_latents(self.vae.encode(vid.unsqueeze(0)), generator, sample_mode='argmax') for vid in video]
        ...
        return (init_latents - latents_mean) * latents_std
    ...
    def __call__(
        ...
        video_latent = self.encode_video(video, generator, batch_size=batch_size, dtype=torch.float32, device=device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = self.scheduler.step(noise_pred, t, latents, return_dict=False, guide=i < guide_until,
                                              video_latent=video_latent, mask=torch.from_numpy(mask).to(device))[0]
