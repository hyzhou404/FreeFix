...
def generate_interpolated_path(
    ...
    def interp(points, n, k, s):
        ...
        sh = points.shape
        num_original_points = sh[0]
        pts = np.reshape(points, (sh[0], -1))
        k = min(k, sh[0] - 1)
        tck, u_orig = scipy.interpolate.make_splprep(pts.T, k=k, s=s)
        u_final = []
        for i in range(num_original_points - 1):
            u_start = u_orig[i]
            u_end = u_orig[i+1]
            segment_u = np.linspace(u_start, u_end, num=n+2, endpoint=True)[:-1]
            u_final.extend(segment_u)
        u_final.append(u_orig[-1])
        u_final = np.array(u_final)
        new_points = tck(u_final)
        new_points = np.reshape(new_points.T, (-1, sh[1], sh[2]))
        return new_points

    points = poses_to_points(poses, dist=rot_weight)
    new_points = interp(
        points, n_interp, k=spline_degree, s=smoothness
    )
