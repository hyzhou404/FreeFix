...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        ibr_latent: Optional[torch.Tensor] = None,
        ibr_mask: Optional[torch.Tensor] = None,
        ...
        if (guided and prior_latents is not None) or (warp and ibr_latent is not None):
            pred_original_sample = self.fuse_latents(
                pred_original_sample, guided, prior_latents, mask, warp, ibr_latent, ibr_mask
            )
    ...
    def fuse_latents(
        ...
        prior_latents: [frames, D=4, H, W]
        '''
        if guided and prior_latents is not None:
            for i in range(pred_original_sample.ndim - mask.ndim):
                mask = mask.unsqueeze(0)
            mask = resize(mask, pred_original_sample.shape[-2:])
            for i in range(pred_original_sample.ndim - prior_latents.ndim):
                prior_latents = prior_latents.unsqueeze(0)

        if warp and ibr_latent is not None:
            for i in range(pred_original_sample.ndim - ibr_mask.ndim):
                ibr_mask = ibr_mask.unsqueeze(0)
            ibr_mask = (resize(ibr_mask, pred_original_sample.shape[-2:]) > 0.5)[0, 0]
            ibr_latent[:, :, ~ibr_mask] = pred_original_sample[:, :, ~ibr_mask].half()

        if guided and warp:
            pred_original_sample = prior_latents*mask + (1-mask)*(ibr_latent * 0.9 + pred_original_sample * 0.1)
            # pred_original_sample = ibr_latent * 0.9 + pred_original_sample * 0.1
            # pred_original_sample = ibr_latent * 0.2 + pred_original_sample * 0.8
            # pred_original_sample = prior_latents*mask + (1-mask)*ibr_latent
            # pred_original_sample = prior_latents*mask + pred_original_sample*(1-mask)
        elif guided and not warp:
            pred_original_sample = prior_latents*mask + (1-mask)*pred_original_sample
        else:
            pred_original_sample = ibr_latent * 0.9 + pred_original_sample * 0.1
        # pred_original_sample = prior_latents*mask + pred_original_sample*(1-mask)
